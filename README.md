...
- **요일별 가격 트렌드를 품목군별로 비교합니다.** 메인 품목 탭과 카드 딜러·버퍼 선택을 반영해 월~일 가격 수준과 전날 대비 변화율을 보여 줍니다. 일반 체결가·카드 0업·맥스업 호가는 분리합니다. 일반 아이템은 하루 체결 5건, 카드는 하루 호가 18시간 이상을 유효 관측으로 삼고 월~일 완료 주 4주 이상인 종목에 같은 비중을 줍니다. 가격 수준은 종목별 주평균 100 기준이며, 전날이 결측이면 변화율을 계산하지 않습니다. 표본 부족 시 요일별 유효 관측 수를 공개합니다.
