...
![1시간 VWAP, 최근 체결가와 시각, 호가 갭 열을 보여주는 거래대금순 시세 목록](docs/list.jpg)

![천공의 지배자의 1시간 VWAP, 관측일 대비 가격 위치와 가격·체결 수량 차트를 보여주는 상세 화면](docs/detail.jpg)

2026-09-19 빌드 화면입니다. 가격과 표본 수는 수집·배포 시점에 따라 달라집니다.
...
일반 아이템의 대표 가격은 **데이터 기준 시각 직전 60분의 수량 가중평균(VWAP)** 입니다. `Σ(체결 단가 × 수량) ÷ 총수량`으로 계산하고, 최근 체결가와 거래 시각을 아래에 따로 표시합니다. 1시간 내 관측 체결이 없으면 평균은 비워 둡니다. 상세 화면은 최근 24시간 평균가가 최근 최대 30개 관측일의 일별 VWAP 중 어디에 있는지를 실제 비교 기간과 함께 보여 줍니다. 위치일 뿐 가격 방향의 예측은 아닙니다. 카드는 체결 API로 업그레이드를 구분할 수 없어 단계별 최저 호가를 사용합니다. 사이트는 매시간 갱신됩니다.
...
- **모델 오차가 naive보다 커서 일반 아이템 상세에는 예측을 그리지 않습니다.** 방향 없이 폭만 정하는 과거 변동폭 기반 참고 범위(마지막 완료일 가격 ± 하루 변동폭 80분위수 × √일수)도 결과를 보기 전에 공식을 고정해 같은 롤링 평가로 재구성했지만, 실제 포함률이 목표 80%에 못 미쳐 분석 페이지에만 기록합니다.
- **유랑악단 패키지 탭에서 해체 마진과 주또주 가치를 계산합니다.** 패키지와 구성품 5종의 최근 24시간 VWAP이 모두 있을 때 수수료 3%를 반영합니다. 주또주 계산기는 세라샵 기존·추가 구매 회차, 보관 수량, 코인 교환 한도와 보유분을 반영해 추가 판매 회수액을 계산합니다. 귀속 보상은 개인 사용 가치로 분리하고, 세라·골드는 사용자 환산 기준이 있을 때만 비교합니다. 관측 단가를 직접 수정할 수 있으며 결측은 0으로 대체하지 않습니다. 실제 판매 가능성·확률형 개봉 수익을 보장하지 않습니다. [계산 기준](docs/package-calculator.md)
- **요일별 가격 트렌드를 품목군별로 비교합니다.** 메인 품목 탭과 카드 딜러·버퍼 선택을 반영해 월~일 가격 수준과 전날 대비 변화율을 보여 줍니다. 일반 체결가·카드 0업·맥스업 호가는 분리합니다. 일반 아이템은 하루 체결 5건, 카드는 하루 호가 18시간 이상을 유효 관측으로 삼고 월~일 완료 주 4주 이상인 종목에 같은 비중을 줍니다. 가격 수준은 종목별 주평균 100 기준이며, 전날이 결측이면 변화율을 계산하지 않습니다. 조건을 채운 종목이 없는 품목군에서는 메인 패널을 숨기고, 준비 현황은 분석 페이지 한계 절에 공개합니다.
- **요일 효과의 통계 검정과 아이템 유형 군집은 보류했습니다.** 요일별 관측 패턴과 유의성 판정은 다릅니다. 4주는 표시 기준이며 검정력을 보장하지 않고, 패치·이벤트 영향을 제거한 결과도 아닙니다. 목요일은 비교를 위한 강조이며 메인에서 `p<0.05`·`현재 비유의` 판정은 사용하지 않습니다.

**2026-09-19 00:55 KST 빌드의 검증 예:** 체결가 분석 후보 38종 중 19종이 분산비 검정 조건을 충족했고, Holm 보정 후 반전 신호 5종·지속 신호 0종이었습니다. 1일 뒤 예측은 35종·139건에서 모델 MAPE 5.29%, naive 3.90%, 7일 뒤는 9종·48건에서 12.14%, 6.43%로 모델의 오차가 더 컸습니다. 참고 범위의 실제 포함률은 1일 73.4%, 7일 60.4%였습니다. 이 수치는 해당 시점의 기록이며, 최신 결과는 매시간 재계산됩니다.

**2026-09-16 요일 보정 비교 실험:** 동일한 과거 평가 사례에서 마지막 가격 유지 / 추세만 / 추세+요일의 MAPE는 1일 뒤 **4.233% / 6.412% / 6.469%**(11종·72건), 7일 뒤 **8.396% / 12.528% / 12.245%**(6종·27건)였습니다. 요일 추가의 영향은 예측 거리·종목별로 달랐고 두 추세 모델 모두 전체 평균에서 가격 유지보다 오차가 컸습니다. 통계적 유의성이나 미래 성능을 입증한 결과는 아닙니다. `npm run experiment:forecast`로 재실행할 수 있으며, [평가 조건·종목별 결과·제외 종목](docs/forecast-comparison-2026-09-16.md)을 기록했습니다.
...
[레전더리 화면](https://kimtaehoon1107-gif.github.io/dnf-price-tracker/index.html#legendary-card)에서 일평균 최저호가와 P10을 별도로 보고, [실험 화면](https://kimtaehoon1107-gif.github.io/dnf-price-tracker/research.html)에서 10개 고정 구성 지수와 구성 아이템별 흐름을 비교합니다. 화면 정리를 위해 기본 목록에는 구성 4종 이상인 7개만 두고, 1~3종 구성은 '소수 구성도 보기'로 엽니다. 체결 VWAP과 카드 단계별 호가는 분리합니다.
...
2026-09-20부터 카드의 시간별 마지막 무매물 상태를 반영한 v2 집계로 새 예측을 발행합니다. 기존 v1 예측·평가 기록은 보존하며 남은 평가도 원래 기준으로 마감합니다. 레전더리 165종은 카드별 최저호가·매물 건수·정상/무매물/실패 상태를 실행별 요약으로 저장하고, 부분 조회 실패를 전체 최저가로 발표하지 않습니다.
