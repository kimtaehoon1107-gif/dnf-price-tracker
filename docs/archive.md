...
매시간 빌드는 `BUILD_HISTORY_CACHE=r2`일 때 같은 비공개 R2 버킷의
`research-v1/build-cache/v1/`에 날짜별 **계산 결과**를 gzip으로 저장한다.
장기 보관 원본을 운영 조회에 그대로 섞지 않으므로, 삭제된 운영 행이 차트에 되살아나지 않는다.
기존 장기 보관 객체와 manifest는 수정하지 않는다.

대상은 일봉·시간봉·호가 갭·시간별 잔량·카드 요일 입력·레전더리 지수·분산비 검정 입력과
연구용 일별 가격·재고다. 대표 가격·상태 점검은 계속 현재 DB에서 읽고, 수집 주기는 바꾸지 않는다.

1. 기존 읽기 전용 REPEATABLE READ 트랜잭션 안에서 DB가 쿼리 결과의 날짜별 SHA-256을 계산한다.
2. R2 파일의 내용이 일치하면 재사용한다. 없는 날짜나 바뀐 날짜만 Supabase에서 내려받는다.
3. 과거 정정·백필·삭제·아이템 분류 변경·이동하는 조회 기간도 현재 결과의 해시에 반영된다.
4. DB의 텍스트 표현을 기존 pg 자료형 파서로 복원해 NULL·numeric·bigint의 해석을 유지한다.

첫 빌드는 캐시를 채우므로 절감이 없다. DB 내부 계산량은 줄이지 않으며, 해시 확인과 변경 날짜
조회로 늘어날 수 있다. R2 장애·검증 실패는 빌드를 중단한다. 장애 시 전체 이력을 다시 내려받는
자동 우회는 없다. 이전 배포와 별도 수집 작업은 유지된다. R2 환경변수와 이 스위치를 생략한
로컬 실행은 기존 직접 조회 방식이다.

빌드 로그의 `[history-cache]`는 조회별 재사용/다운로드 날짜 수와 직렬화한 결과 본문 바이트를
기록한다. 이는 실제 청구 전송량이 아니며 해시·메타데이터·프로토콜·다른 작업의 전송은 별도다.
캐시는 쿼리 정의/자료형/날짜별 최신 파일을 덮어쓰며 시간별 사본을 계속 쌓지 않는다.
PostgreSQL 17 임시 DB의 `scripts/test-history-cache.ts`가 실제 사이트 쿼리와 연구 결과의
최초/재사용 동등성, 정정·삭제·기간 변경·오류 처리를 검증한다. 운영 DB로 테스트하지 않는다.
