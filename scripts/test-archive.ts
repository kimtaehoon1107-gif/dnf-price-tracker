...
const currentOnly={...merged.shards[0],...merged.shards[0].sourceObject};
assert.deepEqual(await readRows(store,currentOnly),[changed,late],'DB에서 정리된 행은 현재 입력 재현에 섞지 않음');
const emptyLocal:Store={get:async()=>null,put:async()=>{throw new Error('읽기 전용');}};
const reused=await combine(merged,{...manifest,shards:[currentOnly]},store,emptyLocal,store);
assert.deepEqual(await readRows(store,reused.shards[0]),[a,changed,late],'정리 후 다음 보관의 재사용도 전체 과거를 보존');
