...
//
// SEED에 수십 줄을 손으로 박는 대신 쓴다. 이벤트 관련 아이템처럼 한꺼번에
// 많이 추가해야 할 때를 위한 것.
//
//   node --env-file=.env --experimental-sqlite --no-warnings scripts/track.ts \
//     "^숲속의 유랑악단" 3600 부속품 --exclude "상자|패키지"
//
...
const existing = new Set((db.prepare('SELECT item_id FROM items').all() as Array<{ item_id: string }>)
  .map((r) => r.item_id));
...
for (const r of toAdd) {
  ins.run(r.itemId, r.itemName, r.itemRarity, r.itemTypeDetail, interval, role,
    `track.ts로 일괄 등록 (패턴: ${pattern})`, now);
}
db.exec('COMMIT');

const total = (db.prepare('SELECT COUNT(*) AS n FROM items WHERE tracked = 1').get() as { n: number }).n;
console.log(`${toAdd.length}종 추가. 현재 추적 대상 ${total}종`);
