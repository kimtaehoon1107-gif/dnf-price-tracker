...
async function boot() {
  ...
  document.getElementById('ranking-view').innerHTML = `
    ...
    <div class="cards">
      ...
      <div class="card"><div class="k">24h 환산</div><div class="v">${counts.estimated ?? 0}<small>종</small></div><div class="sub">100건 체결 속도로 추정</div></div>
    </div>
    ...
    ${rows.length ? `<div class="list ranking-list">
      ...
      ${rows.map((row) => {
        ...
        const content = `
          ...
          <span class="basis-tag ${row.basis}" title="${esc(description)}">${label}</span>`;
